...
Endpoints: auth, patient intake, sensor ingest, readings, alerts, simulation
"""
...
from db import query, query_one, execute, conn
...
def intake(payload: IntakeIn):
    ...
    patient_id = execute(
        """INSERT INTO patients
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
        (payload.user_id, payload.personal.name, payload.personal.age,
         payload.personal.weight_kg, payload.personal.height_cm, bmi,
         payload.personal.blood_group, payload.personal.phone),
    )
    ...
    execute(
        """INSERT INTO pregnancy_info
           ...
           VALUES (?, ?, ?, ?, ?)""",
        (patient_id, payload.pregnancy.trimester, payload.pregnancy.expected_due_date,
         payload.pregnancy.gravida, payload.pregnancy.para),
    )
    ...
    execute(
        """INSERT INTO baseline_vitals
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        (patient_id, payload.baseline.systolic, payload.baseline.diastolic,
         payload.baseline.pulse, payload.baseline.spo2, payload.baseline.temp,
         payload.baseline.sleep_quality, payload.baseline.stress_level,
...
# =========================================================
# PATIENTS LIST — must be BEFORE /patients/{id}
# =========================================================
@app.get("/patients/list")
def list_patients(role: str = "mother", user_id: int = 1):
    if role == "mother":
        rows = query("SELECT id, name, age, blood_group FROM patients WHERE user_id=?", (user_id,))
    else:
        rows = query("SELECT id, name, age, blood_group FROM patients")
    return rows
...
def ingest(payload: ReadingIn, x_device_key: str = Header(...)):
    ...
    rid = execute(
        """INSERT INTO readings (patient_id, sensor_type, value, recorded_at)
           VALUES (?, ?, ?, ?)""",
        (patient_id, payload.sensor_type, json.dumps(payload.value), payload.recorded_at),
    )
    ...
    alert = detect(
        patient_id,
        {"sensor_type": payload.sensor_type, "value": payload.value},
        conn,
    )

    return {"ok": True, "reading_id": rid, "alert": alert}
...
def sim_start(scenario: str, patient_id: int = 1, speed: float = 1.0):
    ...
    t = threading.Thread(target=run_scenario, args=(scenario, patient_id, speed), daemon=True)
    t.start()
    return {"ok": True, "scenario": scenario, "patient_id": patient_id, "speed": speed}
